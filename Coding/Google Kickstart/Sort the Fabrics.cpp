...
const ll INF = 1e18;
bool cmp1(pair<string,ll> p1,pair<string,ll> p2){
    if(p1.first != p1.first){
        return lexicographical_compare(all(p1.first),all(p2.first));
    }
    return p1.second < p2.second;
}
bool cmp2(pl p1,pl p2){
    if(p1.first != p2.first){
        return p1.first < p2.first;
    }
    return p1.second < p2.second;
}
ll check(vector<pair<string,ll>> &vec1,vector<pl> &vec2){
    ll ans = 0;
    ll n = vec1.size();
    for(int i=0;i<n;i++){
...
int main()
{
    ...
    sort(all(vec1));
    sort(all(vec2));
    cout<<"Case #"<<j<<": "<<check(vec1,vec2)<<endl;
    }
