...
void solve()
{
    ...
    loop(i,0,m){
        ...
        //if(left[p-1] == v) ct;
        ll above = i+1;
        ll below = m+1-above-1;
        {
            ll calleft =  ((below)*(below+1))/2;
            mleft[left[p-1]] -= calleft;
            left[p-1] = v;
            mleft[left[p-1]] += calleft;
        }
        {
            ll calcright =  (m*(m+1))/2 - sum(above);
            mright[right[p-1]] -= calcright;
            right[p-1] = v;
            mright[right[p-1]] += calcright;
        }
        // debug(mleft)
    ...
    for(auto p : mleft){
        ll ele = p.fr;
        ans += max(mleft[p.fr],mright[p.fr]);
        mleft[p.fr] = 0;
        mright[p.fr] = 0;
    }
    for(auto p : mright){
