...
void solve()
{
    ll n,m,k; cin>>n>>m>>k;
    vvl a(n,vl(m));
    vvl prec(n+1,vl(m+1,0));
    ll sp = 0;
    loop(i,0,n){
        loop(j,0,m){
            cin>>a[i][j];
            sp += a[i][j];
            prec[i+1][j+1] = a[i][j] + prec[i+1][j] + prec[i][j+1] - prec[i][j];
        }
    }
    // debug(prec)
    if(sp < k+1){
        ...
        loop(i,0,n){
            loop(j,0,m){
                ...
                while(l<=r){
                    ...
                    i2 = max(1ll,i+1-mid);
                    j2 = max(1ll,j+1-mid);
                    ll tot = prec[i1][j1] - prec[i2-1][j1] - prec[i1][j2-1] + prec[i2-1][j2-1];
                    // debug(tot)
                    if(tot >= k+1){
                        ans = min(ans,max({i1-i-1,i+1-i2,j1-j-1,j+1-j2}));
                        r = mid-1;
                    }
                    else{
                        l = mid+1;
