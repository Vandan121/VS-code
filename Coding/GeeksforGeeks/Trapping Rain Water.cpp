...
const ll INF = 1e18;
long long trappingWater(vl a, int n){
        // code here
        vector<long long> forward(n),backward(n);
        forward[0]=a[0];
        for(int i=1;i<n;i++)forward[i]=max(a[i],forward[i-1]);
        backward[n-1]=a[n-1];
        for(int i=n-2;i>=0;i--)backward[i]=max(a[i],backward[i+1]);
        long long sum=0;
        for(int i=1;i<=n-2;i++) sum+=max(forward[i],backward[i])-a[i];
        return sum;
