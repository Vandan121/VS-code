...
//*max_element(all(vec))
//count(all(vec),0ll))
bool func1(st &s,st &s1,vvl &inds){
    ll n = l(s);
    ll m = l(s1);
    ll end = -1;
    loop(i,0,m){
        ll x = s1[i]-'0';
        ll ind = upper_bound(all(inds[x]),end)-inds[x].begin();
        if(ind == inds[x].size()) return true;
        end = inds[x][ind];
    }
    return false;
}
void func(ll i,vvl &vec, st &s,bool &ans,st &dict,vvl &inds){
    if(ans) return;
    if(i==vec.size()){
        ans = func1(dict,s,inds);
        return;
    }
    for(auto v : vec[i]){
        s.pb((char)('0'+v));
        func(i+1,vec,s,ans,dict,inds);
        if(ans) return;
        s.pop_back();
    }
}
void solve()
{
    ...
    vvl vec(m);
    loop(i,0,m){
        loop(j,l[i]-'0',r[i]-'0'+1) vec[i].pb(j);
    }
    st temp = "";
    vvl inds(10);
    loop(i,0,n){
