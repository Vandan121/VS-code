...
void solve()
{
    ...
    vector<pl> coor1 = {{-1,0},{1,0}};
    vector<pl> coor2 = {{0,-1},{0,1}};
    ll ones = 0;
    ll min_l = 3;
    loop(i,0,n){
        loop(j,0,m){
            ...
            for(int i1 = 0;i1<2;i1++){
                ...
                for(int j1=0;j1<2;j1++){
                    ll x1 = i + coor2[j1].fr;
                    ll y1 = j + coor2[j1].sc;
                    if(check(n,m,x,y) && check(n,m,x1,y1)){
                        ll temp = grid[i][j] + grid[x][y] + grid[x1][y1] - '0' - '0' - '0';
                        min_l = min(min_l,temp);
                    }
                }
            }
        }
