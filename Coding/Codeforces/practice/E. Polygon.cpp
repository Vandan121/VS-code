...
void dfs(vector<string> &a, vvl &dp, ll i,ll j){
    dp[i][j] = 1;
    if(check(i-1,j,a.size()) && a[i-1][j] == '1' && dp[i-1][j] == 0) dfs(a,dp,i-1,j);
    if(check(i,j-1,a.size()) && a[i][j-1] == '1' && dp[i][j-1] == 0) dfs(a,dp,i,j-1);
}
