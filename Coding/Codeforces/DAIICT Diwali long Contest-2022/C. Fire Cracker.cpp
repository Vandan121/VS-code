    ...
    while(s.size() && ans<=2*n){
        ...
        if(it.sc>=5000){
            ...
            loop(i,0,n){
                if(a[i][ind] != b[i][ind]){
                    a[i][ind] = b[i][ind];
                    s.erase(s.find({-m[1000+i],1000+i}));
                    m[1000+i]--;
                    if(m[1000+i])s.insert({-m[1000+i],1000+i});
                    m[it.sc]--;
                }
                else{
                    a[i][ind] = !(a[i][ind]-'0')+'0';
                    if(m[1000+i]) s.erase(s.find({-m[1000+i],1000+i}));
                    m[1000+i]++;
                    s.insert({-m[1000+i],1000+i});
                    m[it.sc]++;
                }
            }
        }
            ...
            loop(i,0,n){
                if(a[ind][i] != b[ind][i]){
                    a[ind][i] = b[ind][i];
                    s.erase(s.find({-m[5000+i],5000+i}));
                    m[5000+i]--;
                    if(m[5000+i])s.insert({-m[5000+i],5000+i});
                    m[it.sc]--;
                }
                else{
                    a[ind][i] = !(a[ind][i]-'0')+'0';
                    if(m[5000+i]) s.erase(s.find({-m[5000+i],5000+i}));
                    m[5000+i]++;
                    s.insert({-m[5000+i],5000+i});
                    m[it.sc]++;
                }
            }
        }
        if(m[it.sc]) s.insert({-m[it.sc],it.sc});
        ans++;
        if(s.size()==0){
            cout<<ans<<endl;
            return;
