...
const ll INF = 1e18;
vl dfs(int i,vector<vl> &graph,ll &ans,vector<vl> &bw,string& s,vl &vis){
    vis[i] = 1;
    if(s[i-1]=='B') bw[i][0]++;
    if(s[i-1]=='W') bw[i][1]++;
    for(auto x:graph[i]){
        if(vis[x] == 0){
            vl temp = dfs(x,graph,ans,bw,s,vis);
            bw[i][0] += temp[0];
            bw[i][1] += temp[1];
        }
    }
    if(bw[i][0] == bw[i][1]) ans++;
    return bw[i];
}
void solve()
{
    ...
    string s; cin>>s;
    // debug(graph)
    vector<vl> bw(n+1,vl (2));
    vl vis(n+1,0);
    ll ans = 0;
    dfs(1,graph,ans,bw,s,vis);
    cout<<ans<<endl;
