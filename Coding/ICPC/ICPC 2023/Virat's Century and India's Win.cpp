...
int dp[151][61][11][251][2][2];
ll modinv6 = 166666668;

ll func_p(ll n,ll b,ll w,ll x,ll s,ll ko){
    if(n<=0){
        if(x>=100){
            return 1;
        }
    ...
    //     s ^= 1;
    // }
    if(dp[n][b][w][x][s][ko] != -1){
        return dp[n][b][w][x][s][ko];
    }
    ll ans = 0;
    bool ok = b%6 == 1;
    // if(s==1){
        // ans = (ans + func_p(n,b-1,w+1,x,s,1))%mod;
        ans = (ans + func_p(n-1,b-1,w,min(300*1ll,x+(!ko)*s*1),(ok ? s:!s),ko))%mod;
        ans = (ans + func_p(n-2,b-1,w,min(300*1ll,x+(!ko)*s*2),(ok ? !s:s),ko))%mod;
        ans = (ans + func_p(n-3,b-1,w,min(300*1ll,x+(!ko)*s*3),(ok ? s:!s),ko))%mod;
        ans = (ans + func_p(n-4,b-1,w,min(300*1ll,x+(!ko)*s*4),(ok ? !s:s),ko))%mod;
        ans = (ans + func_p(n-6,b-1,w,min(300*1ll,x+(!ko)*s*6),(ok ? !s:s),ko))%mod;
    // }
    // else{
        // ans = (ans + func_p(n,b-1,w+1,x,s,ko))%mod;
        // ans = (ans + func_p(n-1,b-1,w,x,!s,ko))%mod;
        // ans = (ans + func_p(n-2,b-1,w,x,s,ko))%mod;
        // ans = (ans + func_p(n-3,b-1,w,x,!s,ko))%mod;
        // ans = (ans + func_p(n-4,b-1,w,x,s,ko))%mod;
        // ans = (ans + func_p(n-6,b-1,w,x,s,ko))%mod;
    // }
    if(!ko && s)   {
        if(x>=100)
        ans = (ans + func_p(n,b-1,w+1,x,(ok ? !s:s),1))%mod;
    }
    else if(!ko){
        ans = (ans + func_p(n,b-1,w+1,x,(ok ? !s:s),ko))%mod;
    }
    else if(x>=100) ans = (ans + func_p(n,b-1,w+1,x,(ok ? !s:s),1))%mod;
    ans = (ans*modinv6)%mod;
    return dp[n][b][w][x][s][ko] = ans;
}
...
//     memset(dp,-1,sizeof dp);
    ll n,b,w,x; cin>>n>>b>>w>>x;
    ll p = func_p(n,b,w,x,1,0);
    cout<<p<<endl;
    // memset(dp,-1,sizeof dp);
    // ll q = func_q(n,b,w,x,1,0);
    // cout<<q<<endl;
    // cout<<(p*q)%mod<<endl;
    // ll q_inv = binexp(21,mod-2,mod);
    // cout<<q_inv<<endl;

}


ll func_q(ll n,ll b,ll w,ll x,ll s,ll ko){
    if(n<=0){
        return 1;
    }
    if(w>=10){
    ...
    ll ans = 0;
    ans = (ans + (func_q(n,b-1,w+1,x,s,1)));
    ans = (ans + (func_q(n-1,b-1,w,min(100*1ll,x+1),!s,ko)));
    ans = (ans + (func_q(n-2,b-1,w,min(100*1ll,x+2),s,ko)));
    ans = (ans + (func_q(n-3,b-1,w,min(100*1ll,x+3),!s,ko)));
    ans = (ans + (func_q(n-4,b-1,w,min(100*1ll,x+4),s,ko)));
    ans = (ans + (func_q(n-6,b-1,w,min(100*1ll,x+6),s,ko)));
    return dp[n][b][w][x][s][ko] = ans;
}
int main()
{
