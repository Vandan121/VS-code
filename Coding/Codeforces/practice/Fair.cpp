...
void dfs(vl &goods,vector<vl> &graph,vector<vl> &all_costs){
    int n = graph.size();
    vl vis(n,0);
...
void solve()
{
    ...
    vl ans(n);
    vector<vl> all_costs(n+1);
    for(int i=1;i<=s;i++){
        dfs(goods[i],graph,all_costs);
    }
    for(int i=0;i<n;i++){
        ans[i] = costs(all_costs[i+1],k);
        cout<<ans[i]<<" ";
    }
    cout<<endl;
