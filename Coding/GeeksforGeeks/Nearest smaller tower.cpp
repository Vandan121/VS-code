...
public:
    vector<int> nearestSmallerTower(vector<int> arr)
    {
        ...
        for(int i=n-1;i>=0;i--){
            ...
            if(!s1.empty()){
                if(ans[i] == -1) ans[i] = s1.top().second;
                else{
                    if(abs(ans[i]-i) > abs(i - s1.top().second)) ans[i] = s1.top().second;
                    else if(abs(ans[i]-i) == abs(i - s1.top().second) && arr[ans[i]] > arr[s1.top().second]) ans[i] = s1.top().second;
                }
            }
            s1.push({arr[i],i});
        }
