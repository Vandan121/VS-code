...
vl rot(vl &a,ll r){
    ...
    reverse(all(ans));
    for(auto e : a) ans.pb(e);
    return ans;

}
void func(vvl &grid,ll i,ll j,mll &m,ll r){
    // vl vec;
    vpl ind;
    ll n = grid.size();
    ll m1 = grid[0].size();
    ...
    // debug(ind)
    vl vec;
    for(auto p : ind) vec.pb({grid[p.fr][p.sc]});
    vl ans = rot(vec,r);
    ll n1 = ans.size();
    loop(i,0,n1){
        grid[ind[i].fr][ind[i].sc] = ans[i];
    }
}
void solve()
{
