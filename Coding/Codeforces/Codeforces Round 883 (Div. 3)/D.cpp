...
void solve()
{
    ...
    vector<long double> a(n); in(a,n);
    long double  triangle = (h*d*0.5);
    long double  ans = triangle;
    long double  ratio = d*1.0/h;
    loop(i,0,n-1){
        if(a[i+1] - a[i] >= h){
            ans += triangle;
        }
        else{
            long double  h1 = a[i+1] - a[i];
            long double  b1 = ratio*h1;
            b1 = d - b1;
            long double ans1 = ((b1+d)*h1)/2.0;
            ans += ans1;
        }
    }
