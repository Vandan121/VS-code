...
public:
    ...
    // board[x][y] = s[i];
    void dfs(vector<vector<char> >& board,string s,int i,int x,int y){
        cerr<<s<<" "<<i<<" "<<x<<" "<<y<<endl;
        if(dp[x][y]) return;
        dp[x][y] = 1;
        int n = s.length();
        ...
        for(int x1 = -1;x1 <2;x1++){
            for(int y1 = -1;y1<2;y1++){
                ...
                if(check(x+x1,y+y1,board.size()) && board[x+x1][y+y1]==s[i+1]){
                    dfs(board,s,i+1,x+x1,y+y1);
                }
            }
        }
        dp[x][y] = 0;
	...
	vector<string> wordBoggle(vector<vector<char> >& board, vector<string>& dic) {
	    ...
	    for(int s=0;s<dic.size();s++){
	        for(int i=0;i<n;i++){
	            for(int j=0;j<n;j++){
                    // cout<<board[i][j]<<" ";
	                if(board[i][j] == dic[s][0] &&(ans.empty() || (ans.back()!=dic[s])) ){
                        debug(ans)
	                    dfs(board,dic[s],0,i,j);
	                }
	            }
                cout<<endl;
	        }
