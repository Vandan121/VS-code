...
int main()
{
    ...
    loop(i, 0, n)
    {
        ...
        if (diff > 0)
            ...
            dif_n.push_back(diff);
    }
    sort(dif_n.begin(),dif_n.end());
    ll ans=0,i=0;
    while(i<dif_p.size())
    {
        ll temp = (ll)(lower_bound(all(dif_n),1-dif_p[i])-dif_n.begin());
        ans += (ll)dif_n.size()-temp;
        i++;
    }
    if(dif_p.size() > 1)
    {
        ans += ((ll)dif_p.size()*((ll)dif_p.size()-1))/2;
    } 
    cout<<ans<<endl;
