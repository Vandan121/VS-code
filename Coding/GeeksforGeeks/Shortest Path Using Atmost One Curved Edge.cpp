...
class Solution {
  public:
    void dij(vector<vector<pair<int,int>>> &graph,int i,vector<int> &dist){
        set<pair<int,int>> q;
        q.insert({0,i});
        dist[i] = 0;
        while(q.size()){
            ...
            for(auto e : graph[it.second]){
                if(dist[e.first] > dist[it.second] + e.second){
                    dist[e.first] = dist[it.second] + e.second;
    ...
    int shortestPath(int n, int m, int a, int b, vector<vector<int>> &edges) {
        ...
        vector<int> dista(n+1,1e9);
        vector<int> distb(n+1,1e9);
        dij(graph,a,dista);
        dij(graph,b,distb);
        int ans = dista[b];
        for(auto e : edges){
                ans = min(ans,dista[e[0]]+distb[e[1]]+e[3]);
                ans = min(ans,dista[e[1]]+distb[e[0]]+e[3]);
        }
