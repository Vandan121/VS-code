...
void solve()
{
    ...
    while(q.size()){
        ...
        while(qs--){
            ...
            int ab1 = it.second[2];
            int ba1 = it.second[3];
            if((ind == n) && (a==a1) && (b==b1) && (ab==ab1) && (ba==ba1)){
                cout<<"YES"<<endl;
                return;
            }
            if(s[ind]=='A'){
                if(a1 + 1 <=a){
                    q.push({ind+1,{a1+1,b1,ab1,ba1}});
                }
                if(ind < n-1 && s[ind+1]=='B' && ab1+1<=ab){
                    q.push({ind+2,{a1,b1,ab1+1,ba1}});
                }
            }
            if(s[ind]=='B'){
                if(b1 + 1 <=b){
                    q.push({ind+1,{a1,b1+1,ab1,ba1}});
                }
                if(ind < n-1 && s[ind+1]=='A' && ba1+1<=ba){
                    q.push({ind+2,{a1,b1,ab1,ba1+1}});
                }
            }
        }
    }
