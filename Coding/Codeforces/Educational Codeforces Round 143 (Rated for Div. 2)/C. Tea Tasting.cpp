...
//*max_element(all(vec))
//count(all(vec),0ll))
ll func(vl &vec,ll key,ll l,ll sum){
    ll n = vec.size();
    ll r = n-1;
    ll ans;
    if(key >= vec[n-1]-sum) return -1;
    while(l<=r){
        ll mid = (l+r)/2;
        if(vec[mid]-sum < key){
            l = mid+1;
...
void solve()
{
    ...
    loop(i,0,n){
        ...
        if(ind == -1){
            ...
            if(pref[ind]-sum == a[i]){
                ...
                freq[i+1]++;
                freq[ind]--;
                ans[ind-1] += (a[i] - (pref[ind-1]-sum))*1ll;
            }
        }
        sum = pref[i+1];
    }
    ...
    loop(i,1,n+1){
        sum += freq[i];
        ans[i-1] += 1ll*b[i-1]*sum;
    }
    for(auto a1 : ans) cout<<a1<<" ";
    cout<<endl;
}
int main()
{
