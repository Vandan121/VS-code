...
vl h1(1000010);
vl p(1000010);
ll calc_hash(ll i,ll j){
    if(i==0) return h1[j];
    return (h1[j] - (((h1[i-1])*(p[j-i+1]))%B) + B)%B;
}
ll comp(st &s,ll i,ll j,ll n){
    ll l = 1;
    ll r = n;
    ll ans = 0;
    while(l<=r){
        ll mid = (l+r)/2;
        ll h1 = (calc_hash(i,i+mid-1)+B)%B;
        ll h2 = (calc_hash(j,j+mid-1)+B)%B;
        if(h1 == h2){
            ans = mid;
            l = mid+1;
        }
