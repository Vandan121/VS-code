...
const ll INF = 1e18;
bool check(int i,int j,int n,int l,int r){
    if(i<0 || j<l || i>=n || j>r) {
        // cerr<<i<<" "<<j<<endl;
        return false;
    }
...
void solve()
{
    ...
    vector<pl> coor = {{1,0},{0,1},{-1,0},{0,-1}};
    vector<vl> v(n,vl (m,1));
    loop(i,0,n){
        cin>>grid[i];
    }
    ll left_end = max(0LL,c-1-x);
    ll right_end = min(m-1,c-1+y);
    ll ans = 1;
    v[r-1][c-1] = 0;
    queue<pair<pl,pl>> q;
    q.push({{r-1,c-1},{x,y}});
    while(q.size()){
        ...
        if(check(i+1,j,n,left_end,right_end) && grid[i+1][j] == '.' && v[i+1][j]){
            q.push({{i+1,j},{x1,y1}});
            if(v[i+1][j]){
                ans++;
                v[i+1][j]=0;
        ...
        if(check(i,j+1,n,left_end,right_end) && grid[i][j+1] == '.' && y1>0 && v[i][j+1]){
            q.push({{i,j+1},{x1,y1-1}});
            if(v[i][j+1]){
                ans++;
                v[i][j+1]=0;
            }
        }
        if(check(i,j-1,n,left_end,right_end) && grid[i][j-1] == '.' && x1>0 && v[i][j-1]){
            q.push({{i,j-1},{x1-1,y1}});
            if(v[i][j-1]){
                ans++;
                v[i][j-1]=0;
            }
