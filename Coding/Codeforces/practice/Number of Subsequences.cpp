...
void solve()
{
    ...
    loop(i,1,n+1){
        if(s[i-1]=='a'){
            ...
            dp[1][i] = dp[1][i-1];
            dp[2][i] = dp[2][i-1];
            dp[3][i] = (dp[3][i-1] + dp[2][i-1])%MOD1;
        }
        else{
            dp[0][i] = (3*dp[0][i-1])%MOD1;
            dp[1][i] = (3*dp[1][i-1] + dp[0][i-1])%MOD1;
            dp[2][i] = (3*dp[2][i-1] + dp[1][i-1])%MOD1;
            dp[3][i] = (3*dp[3][i-1] + dp[2][i-1])%MOD1;
        }
    } 
    cout<<dp[3][n]<<endl;
}
