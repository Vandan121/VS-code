...
void solve()
{
    ...
    if(n%2) odd.push_back(0);
    for(int i=1;i<=n/2;i++) odd.push_back(i),even.push_back(i);
        ll n1 = n/2;
        if(n1%2){
            loop(i,0,n1-1) even[i] += 524288;
            rloop(i,n1-1,1) even[i] += 262144; 
        }
        else{
            loop(i,0,n1) even[i] += 524288;
        }
    ll a1=0;
    ll a2=0;
    loop(i,0,odd.size()) a1 ^= odd[i];
    loop(i,0,even.size()) a2 ^= even[i];
    for(int i=0;i<n/2;i++){
        cout<<odd[i]<<" "<<even[i]<<" ";
    }
