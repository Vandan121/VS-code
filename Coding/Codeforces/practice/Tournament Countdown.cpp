...
void func(vl &a){
    ...
    vl b;
    if(n==2){
        ll p1 = a.back(); a.pop_back();
        ll p2 = a.back(); a.pop_back();
        ll x = ask(p1,p2);
        if(x==1) b.push_back(p1);
        else b.push_back(p2);
    }
    else{
        while(a.size()){
            vl c(4);
            loop(i,0,4){
                c[i] = a.back(); a.pop_back();
            }
            ll x = ask(c[0],c[3]);
            if(x == 0){
                x = ask(c[1],c[2]);
                if(x == 1) b.push_back(c[1]);
                else b.push_back(c[2]);
            }
            else if(x == 1){
                x = ask(c[0],c[2]);
                if(x == 1) b.push_back(c[0]);
                else b.push_back(c[2]);
            }
            else{
                x = ask(c[1],c[3]);
                if(x == 1) b.push_back(c[1]);
                else b.push_back(c[3]);
            }
        }
    }
    a = b;
