...
void solve()
{
    ...
    while(s1.size()){
        auto it = s1.begin();
        ll x = *it;
        if(x <= max(s[0],s[n-1]) && x >= min(s[0],s[n-1]) && m[x]){
            if(s[0] > s[n-1]) rloop(i,m1[x].size()-1,0) print.pb(m1[x][i]);
            else loop(i,0,m1[x].size()) print.push_back(m1[x][i]);
            steps += m[x];
            m[x] = 0;
            if(prev) ans += abs(prev-x);
            prev = x;
        }
        s1.erase(it);
    }
    if(print[0] != 1) reverse(all(print));
    cout<<ans<<" "<<steps<<endl;
    for(auto &x : print){
        cout<<x<<" ";
    }
