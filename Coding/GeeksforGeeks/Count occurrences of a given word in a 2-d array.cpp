...
class Solution{
    ...
    void func(vector<vector<char> > &mat, string &target,int i,int j,int k,vector<vector<int> > &vis,int &ans){
        int n = mat.size();
        int m = mat[0].size();
        ...
        for(int ind=0;ind<4;ind++){
            int i1 = j+x[ind];
            int j1 = k+y[ind];
            if(check(i1,j1,n,m) && vis[i1][j1] == 0 && target[i] == mat[i1][j1]){
                vis[i1][j1] = 1;
                func(mat,target,i+1,i1,j1,vis,ans);
                vis[i1][j1] = 0;
            }
        }
        vis[j][k] = 0;
