...
void func(vvl &graph,ll i,ll p,vvl &depths,vl &par,ll d){
    par[i] = p;
    depths[d].pb(i);
    for(auto ele : graph[i]){
        if(ele != p) func(graph,ele,i,depths,par,d+1);
    }
}
...
void solve()
{
    ...
    vvl depths(n+1);
    vl par(n+1);
    func(graph,1,0,depths,par,1);
    vl vis(n+1);
    ll ans = 0;
    for(ll i = n;i>=1;i--){
        for(auto ele : depths[i]){
            if(!vis[ele] && par[ele] && !vis[par[ele]]){
                ans++;
                vis[ele] = 1;
                vis[par[ele]] = 1;
            }
