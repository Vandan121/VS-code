...
int minJumps(vector<int>& arr) {
        ...
        for(auto ind : m[arr[n-1]]) ans[ind] = 1;
        ans[n-1] = 0;
        for(int i=n-2;i>=0;i--){
            int num = ans[i];
            if(i > 0) num = min(num,ans[i-1]+1);
            if(i<n-1) num = min(num,ans[i+1]+1);
            // debug(num)
            if(num < ans[i]){
                for(auto ind : m[arr[i]])
                {
                    if(ind < i) ans[ind] = num+1;
                    else if(ind == i) ans[ind] = num;
                }
            }
            debug(ans)
        }
