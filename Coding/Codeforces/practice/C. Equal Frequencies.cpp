...
//*max_element(all(vec))
//count(all(vec),0ll))
void change(string &s, ll ele){
    vector<pl> vec(26);
    loop(i,0,26){
        vec[i] = {max(n/i,m[i]),i};
    }
    sort(rall(vec));
    while(vec.size() > i) vec.pop_back();
    mll m;
    for(auto p : vec) m[p.second] = n/i;
    loop(i,0,n){
        if(!m[s[i]-'a']){
...
void solve()
{
    ...
    loop(i,2,27){
        ...
        while(vec.size() > i) vec.pop_back();
        ll ans1 = 0;
        for(auto p : vec) ans1 += abs(n/i - m[p.second]);
        if(ans1 < ans){
            ans = ans1;
            no_of_ele = i;
        }
