...
const ll INF = 1e18;
// ll knapsack(int i,vl &wt,vl &val,int w){
//     if(w <= 0 || i<0) return 0;
//     ll a = knapsack(i-1,wt,val,w);
//     ll b =0;
//     if(w >= wt[i]) b = val[i]+knapsack(i-1,wt,val,w-wt[i]);
//     return max(a,b);
// }
void solve()
{
    ...
    vector<vl> ans(n+1,vl(sum+1,1e12));
    for(int i=0;i<=n;i++) ans[i][0] = 0;
    for(int i=1;i<=n;i++){
        for(int j=1;j<=sum;j++){
            if(j < val[i-1]) ans[i][j] = ans[i-1][j];
            else {
                ll a = min(ans[i-1][j],wt[i-1]+ans[i-1][j-val[i-1]]);
                if(a<=w) ans[i][j] = a;
            }
        }
    }
