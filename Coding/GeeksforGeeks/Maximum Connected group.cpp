...
class Solution {
  public:
    ...
    void func(vector<vector<int>>& grid,int &count,int num,int i,int j){
        count++;
        grid[i][j] = num;
        int n = grid.size();
        int m = grid[0].size();
        vector<vector<int>> coor = {{1,0},{-1,0},{0,1},{0,-1}};
        for(auto c : coor){
            int i1 = i + c[0];
            int j1 = j + c[1];
            if(check(i1,j1,n,m) && grid[i1][j1] == 1){
                func(grid,count,num,i1,j1);
            }
        }
    }
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<m;j++){
                ...
                for(auto c : coor){
                    int i1 = i + c[0];
                    int j1 = j + c[1];
                    if(check(i1,j1,n,m) && grid[i1][j1] != 0){
                        s.insert(grid[i1][j1]);
                    }
                }
                if(s.size() > 1){
