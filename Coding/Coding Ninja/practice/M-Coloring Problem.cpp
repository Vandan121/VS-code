...
bool func(vector<vector<int>> &graph,int i,int m,int n,vector<int> &colors){
    if(i==n+1) return true;
    set<int> s;
    for(auto e : graph[i]){
        if(colors[e] != -1) s.insert(colors[e]);
    }
    // debug(i)
    // debug(s)
    bool ok = false;
...
string graphColoring(vector<vector<int>> &mat, int m) {
    ...
    if(func(graph,1,m,n,colors)) return "YES";
    else return "NO";
}
...
void solve()
{
    ...
    string ans = graphColoring(mat,m);
    cout<<ans<<endl;
