...
vector<vector<string>> ans;
    bool check(int i,int j,int n,vector<string> &dp){
        int i1 = i-1,j1=j1-1;
        while(i1>=0){
            if(dp[i1][j]=='Q')return false;
            i1--;
        }
        i1 = i-1;
        while(i1>=0 && j1>=0){
            if(dp[i1][j1]=='Q')return false;
            i1--;
            j1--;
        }
        i1=i-1,j1=j+1;
    ...
    void func(int i,vector<string> &dp,int &n){
        if(i==n){
            ans.push_back(dp);
            return;
        } 
    ...
    vector<vector<string>> solveNQueens(int n) {
        ...
        for(int i=0;i<n;i++) dp.push_back(s);
        func(0,dp,n);
        return ans;
    }
void solve()
