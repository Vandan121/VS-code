...
void solve()
{
    ...
    vl b(m); in(b,m);
    vl prefix_a(n+1,0);
    for(int i=1;i<=n;i++) prefix_a[i] = prefix_a[i-1]+a[i-1];
    vl prefix_b(m+1,0);
    for(int i=1;i<=m;i++) prefix_b[i] = prefix_b[i-1]+b[i-1];
    ll ans_a = prefix_a[0];
    for(int i=0;i<=n;i++) ans_a= max(ans_a,prefix_a[i]);
    ll ans_b = prefix_b[0];
    for(int i=0;i<=m;i++) ans_b= max(ans_b,prefix_b[i]);
