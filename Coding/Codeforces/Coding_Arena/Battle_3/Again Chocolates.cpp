...
int main()
{
    ...
    while(t--)
    {
        ...
        cin>>n;
        vl choc(n);
        loop(i,0,n) cin>>choc[i];
        map<ll,ll>m,m1;
        loop(i,0,n) m[choc[i]]++;
        for(auto i:m) m1[i.second]++;
        vector<pair<ll,ll>> ff;
        for(auto i:m1) ff.push_back(make_pair(i.first,i.second));
        ll ans,ffs = ff.size(),mms = m.size();
        ans = ff[0].first*mms;
        loop(i,1,ffs)
        {
            mms -= ff[i-1].second;
            ans = max(ans,ff[i].first*mms);
        }
        cout<<ans<<endl;
