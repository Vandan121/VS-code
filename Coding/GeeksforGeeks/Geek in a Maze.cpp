	...
	int numberOfCells(int n, int m, int r, int c, int u, int d, vector<vector<char>> &mat)
	{
		...
		while(q.size()){
		    ...
		    while(qs--){
                ...
                int d1 = it.second.second;
		        if( y > 0 && not_visited[x][y-1] && mat[x][y-1] != '#'){
                    ans++;
                    q.push({{x,y-1},{u1,d1}});
                    not_visited[x][y-1] = 0;
                }
                if(y < n-1  && not_visited[x][y+1] && mat[x][y+1] != '#'){
                    ans++;
                    q.push({{x,y+1},{u1,d1}});
                    not_visited[x][y+1] = 0;
                }
                if(x > 0 && not_visited[x-1][y] && mat[x-1][y] != '#' && u1 < u){
                    ans++;
                    q.push({{x-1,y},{u1+1,d1}});
                    not_visited[x-1][y] = 0;
                }
                if(x < m-1 && not_visited[x+1][y] && mat[x+1][y] != '#' && d1 < d){
                    ans++;
                    q.push({{x+1,y},{u1,d1+1}});
                    not_visited[x+1][y] = 0;
                }
		    }
		}
