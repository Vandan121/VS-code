...
void func(ll i,vector<pair<ll,ll>> v){
    ...
    // debug(v2)
    if(v.size() >= 2){
        ll anss = v[0].fr&v[1].fr;
        loop(j,0,k) anss^=(1ll<<j);
        ll vll = (v[0].fr^anss)&(v[1].fr^anss);
        if(vll >= ans){
            vv = anss;
            ans = vll;
            p = make_pair(v[0].sc,v[1].sc);
        }
        debug(ans)
        // mp[ans].pb({v[0].sc,v[1].sc});
    }
}
...
void solve()
{
    ...
    mp.clear();
    ans = 0,vv = 0;
    loop(i,0,n) vec.pb({a[i],i});
    func(k-1,vec);
    cout<<p.fr+1<<" "<<p.sc+1<<" "<<vv<<endl;
}
