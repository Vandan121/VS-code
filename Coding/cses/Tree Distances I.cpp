...
void depth_counter(vector<vector<ll>> &graph,ll i,ll p,vl &depth){
    depth[i] = depth[p] + 1;
    for(auto ele : graph[i]){
        if(ele!=p){
            depth_counter(graph,ele,i,depth);
        }
    }
}
vl func(vector<vector<ll>> &graph){
    ll n = 200010;
    vl depth1(n,-1);
    depth_counter(graph,1,0,depth1);
    ll first_end = -1;
    ll d1 = 0;
    loop(i,1,n){
        if(depth1[i] > d1){
    ...
    vl depth2(n,-1);
    depth_counter(graph,first_end,0,depth2);
    ll second_end = -1;
    ll d2 = 0;
    loop(i,1,n){
        if(depth2[i] > d2){
...
void solve()
{
    ...
    vl vec = func(graph);
    vl depth1(n+1,-1);
    depth_counter(graph,vec[0],0,depth1);
    vl depth2(n+1,-1);
    depth_counter(graph,vec[1],0,depth2);
    loop(i,1,n+1){
