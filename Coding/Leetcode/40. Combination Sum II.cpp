...
set<vector<int>> check;
    void func(vector<int> &arr,vector<int> &A,int B,int ind,int sum){
        if(sum==B){
        ...
        int n = A.size();
        if(sum > B || ind==n) return;
        debug(ind)
        debug(arr)
        func(arr,A,B,ind+1,sum);
        arr.push_back(A[ind]);
        func(arr,A,B,ind+1,sum+A[ind]);
        arr.pop_back();
...
void solve()
{
    ...
    int sum=5;
    vector<vi> ans = combinationSum2(a,sum);
    debug(ans)
}
int main()
{
