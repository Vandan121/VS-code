...
class Solution{
    ...
    long long countPairs(int n, int x, vector<int> numbers){
        ...
        vector<int> z(10);
        z[0] =1;
        for(int i=1;i<10;i++) z[i] = 10*z[i-1];
        int digit=log(x)/log(10) +1;
        long long count=0;
        sort(numbers.begin(),numbers.end());
        map<int,int> m;
        for(int i=0;i<n;i++)
        {
            int digit_i=log(numbers[i])/log(10) +1;
            if(digit_i==digit) break;
            m[numbers[i]]++;
        }
        debug(m)
        for(auto i:m)
        {
            ...
            {
                int digit_j=1;
                while(j.first >= z[digit_j]) digit_j++;
                long long temp = (i.first*z[digit_j])+j.first;
                if(temp == x)
                {
                    if(i.first!=j.first) count += 1LL*i.second*j.second;
                    else count += 1LL*i.second*(j.second-1);   
                }
            }
            // cout<<count<<endl;
        }
