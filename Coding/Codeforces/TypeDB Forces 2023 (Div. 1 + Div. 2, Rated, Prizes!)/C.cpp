...
//count(all(vec),0ll))
vl ans1;
void func(vl &vec,ll &ans,ll i,vl &a,ll s){
    ll n = a.size();
    if(i==n-1){
        vec.pb(a[i]);
        vec.pb(0);
    ...
    for(int j=0;j<=a[i];j++){
        ll v1 = j;
        ll v2 = a[i]-j;
        if((v1-s)*(v2-s) < 0) continue;
        vec.pb(v1);
        vec.pb(v2);
        func(vec,ans,i+1,a,s);
        vec.pop_back();
        vec.pop_back();
    }
}
ll func1(vl &a,ll s){
    ...
    ll v1 = max(s,a[n-2]-s);
    ll v2 = min(s,a[n-2]-s);
    if(vec.back()*v1 + v2*a.back() <= vec.back()*v2 + v1*a.back()){
        vec.pb(v1);
        vec.pb(v2);
    }
    vec.pb(a.back());
...
void solve()
{
    ...
    vvl vec1(n,vl(2));
    vec1[1][0] = a[0]*vec[1][0];
    vec1[1][1] = a[0]*vec[1][1];
    loop(i,2,n-1){
        vec1[i][0] = min(vec1[i-1][0] + vec[i][0]*vec[i-1][1],vec1[i-1][1] + vec[i][0]*vec[i-1][0]);
        vec1[i][1] = min(vec1[i-1][1] + vec[i][1]*vec[i-1][0],vec1[i-1][0] + vec[i][1]*vec[i-1][1]);
    }
    cout<<min(vec1[n-2][0]+a[n-1]*vec[n-2][1],vec1[n-2][1]+a[n-1]*vec[n-2][0])<<endl;
