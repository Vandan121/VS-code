...
class Solution {
  public:
    ...
    void bfs(int r,int c,vector<pair<int,int>> &vec,vector<vector<int>>& grid,vector<vector<int>> &vis, int i,int j){
        vector<pair<int,int>> coor = {{-1,0},{1,0},{0,-1},{0,1}};
        vis[i][j] = 1;
        ...
        for(auto c : coor){
            int x = i + c.first;
            int y = j + c.second;
            if(checker(x,y,n,m) && grid[x][y] && !vis[x][y]){
                bfs(r,c,vec,grid,vis,x,y);
            }
        }
    }
