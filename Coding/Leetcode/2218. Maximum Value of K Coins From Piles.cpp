...
//*max_element(all(vec))
//count(all(vec),0ll))
    int helper(vector<vector<ll>>& piles,int k,vector<int> &ind){
        if(k==0) return 0;
        int ans = 0;
        int n = piles.size();
        for(int i=0;i<n;i++){
            if(ind[i]<piles[i].size()){
                int ans1 = piles[i][ind[i]];
                ind[i]++;
                ans1 += helper(piles,k-1,ind);
                ans = max(ans,ans1);
                ind[i]--;
            }
        }
    ...
    int maxValueOfCoins(vector<vector<ll>>& piles, int k) {
        int n = piles.size();
        int ans = 0;
        vector<int> ind(n);
        return helper(piles,k,ind);
    }
void solve()
{
    ll n; cin>>n;
