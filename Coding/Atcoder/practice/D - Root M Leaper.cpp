...
void bfs(vector<vl> &ans,ll m,vpl &vec){
    ll n = ans.size();
    queue<vl> q;
    q.push({0,0,0});
    while(q.size()){
        auto v = q.front();
        q.pop();
        if(ans[v[0]][v[1]] < v[2] ) continue;
        ll x = v[0];
        ll y = v[1];
        for(auto p : vec){
            ll i = p.fr;
            ll j = p.sc;   
            vector<pl> coor = {{x+i,y+j},{x+i,y-j},{x-i,y+j},{x-i,y-j}};
            for(auto c : coor){
                ll x1 = c.fr;
                ll y1 = c.sc;
                if(check(x1,y1,n) && ans[x][y]+1 < ans[x1][y1]){
                    ans[x1][y1] = ans[x][y]+1;
                    q.push({x1,y1,ans[x1][y1]});
                }
            }
        }
    }
