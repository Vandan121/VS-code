...
//count(all(vec),0ll))
ll ind(vl &vec,ll i){
    if(i < vec[0]) return vec[0];
    if(vec.back() < i) return -1;
    ll left = 0,right = vec.size();
    ll ans;
    while(left<=right){
        ll mid = (left+right)/2;
        if(vec[mid] <= i) left = mid+1;
        else{
            ans = vec[mid];
            right = mid-1;
        }
...
void solve()
{
    ...
    ll cur_ind = -1;
    loop(i,0,m){
        if(indexs[b[i]-'a'].size()==0){
            cout<<-1<<endl;
            return;
        }
        ll temp = ind(indexs[b[i]-'a'],cur_ind);
        if(temp==-1){
            ans++;
            cur_ind = -1;
            cur_ind = ind(indexs[b[i]-'a'],cur_ind);
        }
        else{
            cur_ind = temp;
        }
