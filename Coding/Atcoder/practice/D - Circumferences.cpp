...
const ll INF = 1e18;
void dfs(int n,vl &visited,vector<vl> &graph){
    visited[n]=1;
    for(auto &x:graph[n])
    {
...
void solve()
{
    ...
    ll a,b;
    vector<vl> circle(n,vector<ll> (3));
    for(int i=0;i<n;i++){
        cin>>circle[i][0];
        cin>>circle[i][1];
        cin>>circle[i][2];
        if((abs(sx-circle[i][0]))*(abs(sx-circle[i][0]))+(abs(sy-circle[i][1]))*(abs(sy-circle[i][1])) == circle[i][2]*circle[i][2]){
            a = i;
        }
        if((abs(tx-circle[i][0]))*(abs(tx-circle[i][0]))+(abs(ty-circle[i][1]))*(abs(ty-circle[i][1])) == circle[i][2]*circle[i][2]){
            b = i;
        }
    }
    ...
    for(int i=0;i<n;i++){
        for(int j=i+1;j<n;j++){
            ll dist = (abs(circle[i][0]-circle[j][0]))*(abs(circle[i][0]-circle[j][0]))+
                        (abs(circle[i][1]-circle[j][1]))*(abs(circle[i][1]-circle[j][1]));
            if(dist >= (circle[i][2]-circle[j][2])*(circle[i][2]-circle[j][2]) && dist <= (circle[i][2]+circle[j][2])*(circle[i][2]+circle[j][2])){
            graph[i].push_back(j);
            graph[j].push_back(i);
            }
        }
