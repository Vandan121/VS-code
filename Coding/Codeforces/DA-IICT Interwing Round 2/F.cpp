...
vector<int> max_valid;

void compute_max_valid(vector<int>& A) {
    int n = A.size();
    max_valid.resize(n);
    max_valid[0] = 1;
    for (int i = 1; i < n; i++) {
        if (A[i-1] + 1 == A[i]) {
            max_valid[i] = max_valid[i-1] + 1;
        } else {
            max_valid[i] = 1;
        }
    }
