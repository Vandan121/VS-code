...
const ll INF = 1e18;
vector<int> a(1001,INT_MAX);
vector<vl> a1;
ll solve(int k,vl b,vl c,int n)
{
    if(a1[n][k] != -1) return a1[n][k];
    if(n==0) return a1[n][k] = 0;
    if(a[b[n-1]] > k) return a1[n][k] = solve(k,b,c,n-1);
    else{
        return a1[n][k] = max(solve(k,b,c,n-1),c[n-1]+solve(k-a[b[n-1]],b,c,n-1));
    }
}
int main()
{
    ...
    while(t--)
    {
        ...
        a1.clear();
        vector<ll> b1(k+1,-1);
        for(int i=0;i<=n;i++) a1.push_back(b1);
        cout<<solve(k,b,c,n)<<endl;
    }
    return 0;
}