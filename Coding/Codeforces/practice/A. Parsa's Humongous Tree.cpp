...
ll dfs(vvl &graph,vl &l,vl &r,ll i,ll left,ll right,ll p,vector<vector<vector<ll>>> &dp){
    ...
    for(auto ele : graph[i]){
        if(ele != p){
            ll ans1 = dfs(graph,l,r,ele,1,0,i,dp) + abs(var-l[ele]);
            ll ans2 = dfs(graph,l,r,ele,0,1,i,dp) + abs(var-r[ele]);
            ans += max(ans1,ans2);
        }
    }
...
void solve()
{
    ...
    loop(i,1,n+1) cin>>left[i]>>right[i];
    ...
    vector<vvl> dp(n+1,vvl (2,vl (2,-1)));
    ll ans = 0;
    ans = max(ans,dfs(graph,left,right,1,1,0,0,dp));
    ans = max(ans,dfs(graph,left,right,1,0,1,0,dp));
    // debug(dp)
    cout<<ans<<endl;
