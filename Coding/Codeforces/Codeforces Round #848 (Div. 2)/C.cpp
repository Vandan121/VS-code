...
ll func1(vl &vec1){
    ...
    if(len>1 || vec1.size()) ans1 += (len*(len+1))/2;
    return ans1;
}
...
void solve()
{
    ...
    vl ele;
    loop(i,0,26){
        if(m[i].size()) ele.pb(i);
    }
    ll n1 = ele.size();
    vector<vl> all_set = makeCombi(n1,min(n1,k));
    // vector<vl> all_set;
    // debug(all_set)
    ll ans = 0;
    for(auto vec : all_set){
        if(vec.size()-k > 0) continue;
        vl m1(26,0);
        for(auto e : vec){
            m1[ele[e-1]]++;
        }
