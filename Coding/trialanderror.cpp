...
//find_by_order(k) : K-th element in a set (counting from zero).
 bool check(vector<ll> &w,int d,int w1){
        int cnt = 0;
        int sum = 0;
        int n = w.size();
        for(int i=0;i<n;i++){
            if(w[i] > w1) return false;
            sum += w[i];
            if(sum > w1){
                cnt++;
                sum = w[i];
...
void solve()
{
    ...
    // }
    // ct.close();
    vector<vl> all_set = makeCombi(20,5);
    debug(all_set)
    // loop(i,1,101){
    //     int n = i;
    //     int x = 0;
    ...
    //     ll var2 = rand()%100;
    //     ll var3 = rand()%100;
    //     a.pb({var1+var2+var3,var1,var2,var3});
    // }
    // loop(i,0,10){
    //     ll a = rand()%10 + 1;
    //     ll b = rand()%10 + 1;
    ...
    // cout<<1<<endl;
    // cout<<262144<<endl;
    // for(ll i=1;i<=262144 ;i++){
    //     a[i-1] = i;
    // }
    // // debug(a)
    // for(ll i=262144;i>=2;i/=2){
    //     for(ll j=0;j<262144;j+=i){
    //         func(a,j,j+i/2,i/2);
    //     }
    // }
    // ll ans = 0;
    // loop(i,0,262144) cout<<a[i]<<" ";
