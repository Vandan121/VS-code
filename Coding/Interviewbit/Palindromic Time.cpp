...
void solve(string A) {
    int hours=0;
    for(int i=0;i<2;i++) hours = hours*10 + A[i]-'0';
    int minutes=0;
    for(int i=3;i<5;i++) minutes = minutes*10 + A[i]-'0';
    int time=0,expected_min;
    while(hours/10 != minutes%10 || hours%10 != minutes/10)
    {
        expected_min = (hours%10)*10 + hours/10;
        if(minutes>expected_min) {
            time += 60 - minutes;
            hours++;
            minutes=0;
        }
