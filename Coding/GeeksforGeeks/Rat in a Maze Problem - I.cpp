...
class Solution{
    ...
    vector<string> ans;
    int dp[10][10];
    bool check(int i,int j,vector<vector<int>> &m){
        int n = m.size();
        if(i<0 || j<0 || i>=n || j>=n || !m[i][j] || dp[i][j]) return false;
        return true;
    }
    void func(int i,int j,vector<vector<int>> &m,string s){
        debug(s)
        int n = m.size();
        if(i==(n-1) && j==(n-1)){
            ans.push_back(s);
        ...
        dp[i][j] = 1;
        vector<pair<int,int>> coor = {{-1,0},{0,1},{1,0},{0,-1}};
        vector<char> ch = {'U','R','D','L'};
        for(int ind = 0;ind<4;ind++){
            int x = i+coor[ind].first;
            int y = j+coor[ind].second;
            if(check(x,y,m)){
                s.push_back(ch[ind]);
                func(x,y,m,s);
                s.pop_back();
