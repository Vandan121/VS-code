...
class Node{
    ...
    Node(){
        word = false;
        loop(i,0,26) vec.pb(nullptr);
    }
};
ll func(st &s,Node* trie,ll i,vl &dp){
    ll n = l(s);
    if(i==n) return 1;
    ...
    ll ans1 = 0;
    loop(j,i,n){
        if(node1->vec[s[j]-'a'] != nullptr){
            node1 = node1->vec[s[j]-'a'];
            if(node1->word){
                ans1 = (ans1 + func(s,trie,j+1,dp))%MOD1;
            }
        }
        else{
            break;
...
void solve()
{
    ...
    loop(i,0,k){
        ...
        Node* temp = trie;
        for(auto &c : dict[i]){
            if(temp->vec[c-'a'] == nullptr){
                temp->vec[c-'a'] = new Node();
            }
            temp = temp->vec[c-'a'];
        }
        temp->word = true;
