...
//*max_element(all(vec))
//count(all(vec),0ll))
bool cycle(string node, unordered_map<string, int> &vis, unordered_map<string, int> &dfs_vis, unordered_map<string, vector<string>> &adj)
{
    vis[node] = 1;
    dfs_vis[node] = 1;
    for (auto i : adj[node])
...
void solve()
{
    ...
    for (auto i : nodes)
    {
        if (!vis[i])
        {
            unordered_map<string, int> dfs_vis;
            if (cycle(i, vis, dfs_vis, adj))
            {
                cout<<"No"<<endl;
                return;
