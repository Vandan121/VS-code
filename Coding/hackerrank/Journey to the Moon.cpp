...
const ll INF = 1e18;
ll elements(vl &visited,vector<vl> &graph,int ele){
    queue<ll> q;
    q.push(ele);
    visited[ele] = 1;
    ll ans = 0;
...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        ...
        sizes.push_back(elements(visited,graph,i));
    }
    n = sizes.size();
    vl prefix(n);
    prefix[n-1] = sizes[n-1];
    for(int i=n-2;i>=0;i--){
        prefix[i] = sizes[i] + prefix[i+1];
    }
    ll ans = 0;
    for(int i=0;i<n-1;i++) ans += sizes[i]*prefix[i+1];
    cout<<ans<<endl;
}
int main()
{
