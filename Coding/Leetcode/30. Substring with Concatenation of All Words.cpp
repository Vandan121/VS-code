...
const ll INF = 1e18;
bool check(map<string,int> m,vector<string>& words,string &s,int i){
        string s1 = "";
        int n = words.size();
        for(int j = i;j<=i+n*words[0].length();j++){
            if(s1.length() == words[0].length()){
                if(m[s1]){
                    m[s1]--;
                    s1 = s[j];
                }
    ...
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        vector<int> ans;
        for(int i=0;i+n*words[0].length()<=s.length();i++){
            if(check(m,words,s,i)){
                ans.push_back(i);
            }
        }
        return ans;
...
void solve()
{
    ...
    debug(s)
    debug(w)
    vector<int> ans = findSubstring(s,w);
    dis(ans)
}
int main()
{
