...
// using namespace std;
// int dp[71][4901];
// int f(int row, int sum, int k, vector<vector<int>> &grid)
// {
//     if (row == grid.size())
//     {
//         return abs(sum - k);
...
//     {
//         ans = min(ans, f(row + 1, sum + grid[row][i], k, grid));
//         if (!ans)
//         {
...
ll func(vvl &grid,ll i,ll sum,ll k){
    ll n = grid.size();
    if(i == n){
        return abs(k-sum);
    }
    ...
    ll ans = INT_MAX;
    loop(j,0,m){
        ans = min(ans,func(grid,i+1,sum+grid[i][j],k));
    }
    return dp[i][sum] = ans;
}
