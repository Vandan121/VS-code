...
void solve()
{
    ...
    string str="";
    loop(i,1,k+1) str+=48+i;
    set<string> s;
    do{
        s.insert(str);
    }while(next_permutation(all(str)));
    for(auto str : s){
        vector<string> edited_numbers;
        loop(i,0,n){
            string temp = str;
            loop(j,0,k) temp[j] = a[i][str[j]-'1']; 
            edited_numbers.push_back(temp);
        }
        sort(all(edited_numbers));
        ll diff = stoi(edited_numbers[n-1])-stoi(edited_numbers[0]);
        ans = min(ans,diff);
    }
    cout<<ans;
