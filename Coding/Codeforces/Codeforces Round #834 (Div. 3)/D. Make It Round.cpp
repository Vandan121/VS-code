...
void solve()
{
    ...
    rloop(i,18,max(1ll,ten)){
        ll rem_2 = max(0ll,i-two);
        ll rem_5 = max(0ll,i-five);
        if(vec_2[rem_2]*vec_5[rem_5] <= m){
            ll temp = m/(vec_2[rem_2]*vec_5[rem_5]);
            temp *= (vec_2[rem_2]*vec_5[rem_5]);
            cout<<n*temp<<endl;
            return;
        }
    }
