...
void func(vvl &grid,ll i,ll j, mll &num,ll x){
    ll n = grid.size();
    ll m = grid[0].size();
    num[grid[i][j]] = x;
    vector<pl> coor = {{0,-1},{0,1},{1,0},{-1,0}};
    for(auto c : coor){
        ll i1 = i + c.fr;
        ll j1 = j + c.sc;
        if(check(i1,j1,n,m) && grid[i1][j1] != -1 && num[grid[i1][j1]] == 0){
            func(grid,i1,j1,num,x);
        } 
    }
}
...
void solve()
{
    ...
    loop(i,0,n){
        loop(j,0,n){
            if(grid[i][j] != -1 && num[grid[i][j]] == 0){
                func(grid,i,j,num,ele);
                ele++;
            }
        }
    }
    ...
    loop(i,0,n){
        loop(j,0,n){
            if(grid[i][j] != -1){
                ...
                ll x = grid[i][j];
                while(x*f <= maxi){
                    if(num[x*f] && num[x*f] != num[grid[i][j]]){
                        sum = (sum+x*f)%mod;
                    }
                    f++;
                }
