...
public:
    ...
    void func(vector<vector<int>> &mat,int i,int j,vector<vector<int>> &vis){
        int m = mat.size();
        int n = mat.size();
        ...
        while(q.size()){
            ...
            int i1 = it.first;
            int j1 = it.second;
            if(i1 != m-1 && j1 != n-1 || vis[i1][j1]==1) ok = true;
            if(vis[i1][j1] >= 0) continue;
            vis[i1][j1] = -2;
            for(auto c : coor){
                int x1 = i1 + c.first;
                int y1 = j1 + c.second;
                if(check(x1,y1,m,n) && mat[x1][y1] >= mat[i1][j1]){
                    if(vis[x1][y1] == -1){
                        vis[x1][y1] = -2;
                        q.push({x1,y1});
                    }
        ...
        for(auto p : ele){
            if(ok) vis[p.first][p.second] = 1;
            else vis[p.first][p.second] = 0;
        }
